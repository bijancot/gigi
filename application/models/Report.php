<?php
    class Report extends CI_Model{
        ...
        public function getAll(){
            $this->db->select('r.report_id as report_id, u.nisn as nisn, u.name as name, r.day as day, r.status as status, max(rd.created_at) as created_at');
            $this->db->from('user u');
            $this->db->join('report r', 'r.user_nisn = u.nisn');
            $this->db->join('report_detail rd', 'rd.report_id = r.report_id', 'left');
        ...
        public function checkLastOpen($param) {
            $this->db->select('r.report_id as report_id, DATE(r.last_open) as date, r.day as day, r.status as status');
            $this->db->from('report r');
            $this->db->join('user u', 'u.nisn = r.user_nisn', 'left');
            $this->db->where('u.nisn', $param);
            $this->db->where('r.status !=', 'canceled');
